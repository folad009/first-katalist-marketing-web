...
const ServicesSlider1 = ({ services = [] }) => {
	const showableSevices = services?.slice(0, 4);
	const sliderServices = [...showableSevices, ...showableSevices];
	return (
		<Swiper
			slidesPerView={1.3}
			spaceBetween={15}
		...
		>
			{sliderServices?.length
				? sliderServices?.map((service, idx) => (
						<SwiperSlide key={idx}>
