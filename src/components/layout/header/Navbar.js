  ...
  return (
    <nav className="max-lg:hidden">
      <ul className="m-0 flex list-none flex-wrap items-center gap-x-7.5 p-0 xl:max-2xl:gap-x-5.5 lg:max-xl:gap-x-4">
        ...
        {/* FKM Group — simple dropdown */}
        <li className="group/nav relative z-1">
          <Link
            href={groupNav?.path ? groupNav.path : "/fk-group"}
            className={cn(topLink(groupNav?.isActive), "pe-5")}
          >
            {groupNav?.name ? groupNav.name : "FKM Group"}
            <Caret />
          </Link>
          <ul className={cn(dropdownPanel, "w-67.5 py-2.5")}>
            {groupNav?.submenu?.length
